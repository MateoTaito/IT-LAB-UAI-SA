...
import API_Base from "../lib/AxiosBase";
...
export async function createUser(
  ...
  try {
    ...
    onStateChange?.(
      UserCreationState.CREATING_USER,
    ...
    onStateChange?.(
      UserCreationState.WAITING_FINGERPRINT,
      "Please provide fingerprint for enrollment...",
    );

    // Step 3: Enroll fingerprint
    onStateChange?.(
      UserCreationState.ENROLLING_FINGERPRINT,
      "Enrolling fingerprint...",
    );
    const fingerprint_response = await API_Fingerprint.post(
      "/enrollment/user",
      {
    ...
    onStateChange?.(
      ...
      "User created and fingerprint enrolled successfully!",
    );
    return createdUser;
  } catch (error) {
    onStateChange?.(
      UserCreationState.ERROR,
      "Failed to create user or enroll fingerprint",
    );
...
export async function deleteUser(email: string): Promise<{ message: string }> {
  try {
    ...
    try {
      ...
      console.warn(
        "Failed to delete fingerprint data, but user was deleted:",
        fingerprintError,
      );
    }
...
export async function getAdminInfo(userId: string): Promise<User> {
  try {
    const response = await API_Base.get(`/${userId}`);
    return response.data;
  } catch (error) {
 ...
 * @returns Success message
 */
export async function deleteUserCareer(
  email: string,
  career: string,
): Promise<{ message: string }> {
  try {
